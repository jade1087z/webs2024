...
// 내가 쓴 다이어리 정보 가져오기
$diarySql = "SELECT * FROM drinkBoard WHERE memberId = '{$myMemberId}' AND boardCategory = '마이 다이어리'";
$diaryResult = $connect->query($diarySql);
$diaryInfo = $diaryResult->fetch_array(MYSQLI_ASSOC);
$diaryRegTime = date("Y.m.d", strtotime($diaryInfo['boardRegTime']));
$realDiaryRegTime = date("Y.m.d", $diaryInfo['boardRegTime']);

// 내가 쓴 자유게시판 정보 가져오기
$boardSql = "SELECT * FROM drinkBoard WHERE memberId = '{$myMemberId}' AND boardCategory = '자유게시판'";
...
$boardRegTime = date('Y.m.d', $dateInfo['boardRegTime']);
                ...
                <div class="best_list boxStyle roundCorner shaDow">
                    ...
                    <div id="mypage_diary" class="tab-content">
                        <div class="today_diary">
                            <h2><?= $realDiaryRegTime ?></h2>
                            <?php if ($diaryRegTime !== $today) { ?>
