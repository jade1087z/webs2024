...
if (isset($_GET['searchOption']) && isset($_GET['searchQuery'])) {
    ...
    // 결과에 따른 개수 불러오기
    if ($searchOption === '게시글') {
        $searchSql = "SELECT count(boardId) FROM drinkBoard WHERE boardDelete = 1 AND boardCategory = '자유게시판' AND boardContentS LIKE '%{$searchQuery}%'";
    } else if ($searchOption === '제목만') {
        $searchSql = "SELECT count(boardId) FROM drinkBoard WHERE boardDelete = 1 AND boardCategory = '자유게시판' AND boardTitle LIKE '%{$searchQuery}%'";
    } else if ($searchOption === '작성자') {
        $searchSql = "SELECT count(boardId) FROM drinkBoard WHERE boardDelete = 1 AND boardCategory = '자유게시판' AND boardAuthor LIKE '%{$searchQuery}%'";
    }
    $result = $connect->query($searchSql);
    if (!$result) {
    ...
    // 불러올 값 정렬 
    if ($searchOption === '게시글') {
        $boardSql = "SELECT * FROM drinkBoard WHERE boardDelete = 1 AND boardCategory = '자유게시판' AND boardContentS LIKE '%{$searchQuery}%' ORDER BY boardId DESC LIMIT {$viewLimit}, $viewNum";
    } else if ($searchOption === '제목만') {
        $boardSql = "SELECT * FROM drinkBoard WHERE boardDelete = 1 AND boardCategory = '자유게시판' AND boardTitle LIKE '%{$searchQuery}%' ORDER BY boardId DESC LIMIT {$viewLimit}, $viewNum";
    } else if ($searchOption === '작성자') {
        $boardSql = "SELECT * FROM drinkBoard WHERE boardDelete = 1 AND boardCategory = '자유게시판' AND boardAuthor LIKE '%{$searchQuery}%' ORDER BY boardId DESC LIMIT {$viewLimit}, $viewNum";
    }
        ...
        <main id="contents_area">
            <section id="main_contents">
                ...
                <div class="best_list boxStyle roundCorner shaDow">
                    <h4>자유 게시판</h4>
                    <h3>검색 결과 <span class="search_num"><?= $boardTotalCount ?></span> 개</h3>
                    <?php if ($boardInfo->num_rows == 0) {
                    ?>
                        <h2>영인님 디자인 좀 해주세요.</h2>
                    <?php } else { ?>
                        <ul class="board_w100">
                            <?php foreach ($boardInfo as $board) { ?>
                                <li>
                                    <a href="board_view.php?boardId=<?= $board['boardId'] ?>" data-boardId="<?= $board['boardId'] ?>">
                                        <div class="board_info">
                                            <div class="board_title textCut"><?= $board['boardTitle'] ?></div>
                                            <div class="board_author textCut"><?= $board['boardAuthor'] ?></div>
                                            <div class="board_date"><?= date('Y.m.d', $board['regtime']) ?></div>
